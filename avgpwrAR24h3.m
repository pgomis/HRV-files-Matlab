function ENER=avgpwrAR24h3(data,nmin,Fs,bands,Nfft,na)
...
%  for each 5-min segment (LF and HF) (uses 5-min segments for LF and HF)
%  IF 5-MIN SEGMENT IS EMPTY, SKIPS IT (improved)
% The routine is degigned to compute standard HRV spectral indices (24h)
% The R-R interval time series is resampled at "fs" Hz.
%
% ENER = avgpwrAR24h3(data,nmin,Fs,bandes,Nfft,na)
% Input data: 2 columns = [time values]. After resampled, the series is 
% detrended.
% nmin: number of minutes of HRV spectral short-segments, (default: nmin=5) 
...
% indTot = f> 1.5e-5 & f<0.5;
% indULF = f> 1.5e-5 & f < bands(1,1);
indTot = f> 0 & f<0.5;
